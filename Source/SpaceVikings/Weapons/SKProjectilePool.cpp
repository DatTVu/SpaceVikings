	...
	if (PooledObjectSubClass != nullptr) {
		...
		if (World != nullptr) {
			for (int i = 0; i < PoolSize; ++i) {
				ASKProjectileBase* PooledProjectile = World->SpawnActor<ASKProjectileBase>(PooledObjectSubClass, FVector(1000, 1000, 1000), FRotator(0, 0, 0));
...
ASKProjectileBase* USKProjectilePool::SpawnPooledProjectile() {
	for (ASKProjectileBase* PooledProjectile : ActorPool) {
		if (PooledProjectile != nullptr && !PooledProjectile->IsActive()) {
			PooledProjectile->TeleportTo(FVector(1000, 1000, 1000), FRotator(0, 0, 0));
...
void USKProjectilePool::OnPooledActorDespawn(ASKProjectileBase* PooledActor) {
	PooledActor->TeleportTo(FVector(1000, 1000, 1000), FRotator(0, 0, 0));
